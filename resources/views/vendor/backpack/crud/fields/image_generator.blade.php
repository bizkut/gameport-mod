  <div class="form-group col-md-12 image" data-preview="#{{ $field['name'] }}" data-aspectRatio="{{ isset($field['aspect_ratio']) ? $field['aspect_ratio'] : 0 }}" data-crop="{{ isset($field['crop']) ? $field['crop'] : false }}" @include('crud::inc.field_wrapper_attributes')>
    ...
    <div class="row">
        <div class="col-sm-6" style="margin-bottom: 20px;">
            <img id="mainImage" src="{{ url(old($field['name']) ? old($field['name']) : (isset($field['value']) ? 'images/cover/' . $field['value'] : (isset($field['default']) ? $field['default'] : '') )) }}">
        </div>
    ...
    <div class="btn-group">
        <label class="btn btn-primary btn-file">
            ...
            <input type="hidden" id="hiddenImage" name="{{ $field['name'] }}" value="{{ isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : '') }}">
        </label>
        @if(isset($field['crop']) && $field['crop'])
            ...
            jQuery(document).ready(function($) {
                ...
                $('.form-group.image').each(function(index){
                    ...
                    if(crop){
                      ...
                      /** $mainImage.cropper(options).cropper("reset", true).cropper('replace', 'http://www.giantbomb.com/api/image/scale_large/2193420-mario_all_stars_world1box.jpg'); **/
