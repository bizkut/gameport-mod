...
class MessagesController extends Controller
{
    ...
    public function index()
    {
        ...
        if ( config('settings.messenger_color_counts') ) {
            $count = config('settings.messenger_color_counts') > $count ? $count : config('settings.messenger_color_counts');
            for ($index = 1; $index <= $count; $index++) {
                array_push($colors, config('settings.messenger' . $index . '_color'));
            }
        }
        if ($threads->isEmpty()) {
